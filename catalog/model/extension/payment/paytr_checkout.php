...
class ModelExtensionPaymentPaytrCheckout extends Model
{
    public function getMethod($address, $total)
    {
        ...
        if ($this->config->get('payment_paytr_checkout_total') > 0 && $this->config->get('payment_paytr_checkout_total') > $total) {
            $status = false;
        } elseif (!$this->config->get('payment_paytr_checkout_geo_zone_id')) {
            $status = true;
